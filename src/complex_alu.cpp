...
void complex_alu :: prc_complex_alu ()
{
	...
	{
		z_r.write(((x_r.read() * y_r.read()) + (x_im.read() * y_im.read()))/((y_r.read()*y_r.read()) + (y_im.read()*y_im.read())));
		z_im.write(((x_im.read() * y_r.read()) - (x_r.read() * y_im.read()))/((y_r.read()*y_r.read()) + (y_im.read()*y_im.read())));
	}
}
