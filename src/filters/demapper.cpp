...
Demapper::Demapper(MCS  mcs,
                   bool hard) :
    FilterChainElement("DEMAPPER"),
    m_value(),
    m_inputValid(false),
    m_constellations(),
    m_bitsPerSymbol(0),
    m_hard(hard),
    m_awgnVariance(1.0), // XXX will need a way to adjust at runtime based on
...
void Demapper::tick(void)
{
    ...
    for (size_t j = 0; j < m_bitsPerSymbol; j++) {
        ...
        for (auto it = m_constellations.begin(); it != m_constellations.end();
             ++it) {
            double incr = calcLLRIncrement(value,
                                           it->second.toComplexDouble(),
                                           m_awgnVariance);
...
double Demapper::calcLLRIncrement(const ComplexDouble& rxSymbol,
