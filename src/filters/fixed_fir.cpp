...
FixedFIR<COEFF_T, IO_T, COEFFI_T>::FixedFIR(const std::vector<COEFFI_T>& coeffs,
                                            const FixedFIRConfig       & conf) :
    FilterChainElement("FixedFIR"),
    m_coeffs(),
    m_delayLine(),
    m_output(conf.wlOut, conf.iwlOut, SLFixPoint::QUANT_RND_HALF_UP,
    ...
    m_filterIteration(0)
{
    ...
    if ((m_rateAdj >= -1) && (m_rateAdj <= 1)) {
        // -1, 0, and 1 all mean "no rate change" use 1 as the canonical value
    ...
    if (m_rateAdj > 1) {
        ...
        size_t coeffExtention =
            (m_rateAdj - coeffs.size() % m_rateAdj) % m_rateAdj;
        m_coeffs.resize(coeffs.size() +  coeffExtention);
        log_debug(
            "Interpolation by a factor of %d. Zero padding with %d additional coefficients for polyphase operations.",
            m_rateAdj,
            coeffExtention);
    } else {
    ...
    // Construct the fixed point coefficients with scaling of 2^(wlCoeff -
    // maxIntWidth)
    for (unsigned int i = 0; i < coeffs.size(); i++) {
        m_coeffs[i].setFormat(conf.wlCoeff,
                              maxIntWidth,
    ...
    // If we are upsampling, our delay line size is a fraction of the
    // coefficient size
    if (m_rateAdj > 1) {
        delaySize /= m_rateAdj;
    }
    m_delayLine =
        CircularBuffer<IO_T>(delaySize,
                             IO_T(conf.wlDelay, conf.iwlDelay,
                                  SLFixPoint::QUANT_RND_HALF_UP,
                                  SLFixPoint::OVERFLOW_SATURATE));

    if (m_rateAdj < -1) {
        log_debug("Decimation filter downsampling by a factor of %d.",
                  abs(m_rateAdj));
    }

    // See http://www.digitalsignallabs.com/fir.pdf for analysis on FIR bit
    // growth
    // unsigned worstCaseBitGrowth =
    // static_cast<unsigned>(ceil(log2(coeffArea)));
    ...
    m_accum.setFormat(52,
                      conf.iwlOut + m_coeffs[0].iwl(),
                      SLFixPoint::QUANT_RND_HALF_UP,
                      SLFixPoint::OVERFLOW_SATURATE);

    // m_accum.setFormat(bitsRequired, maxIntWidth + worstCaseBitGrowth,
    // SLFixPoint::QUANT_RND_HALF_UP, SLFixPoint::OVERFLOW_SATURATE);
    // log_debug("FIR coefficient format is Q%d.%d, Accumulator format is
    // Q%d.%d. Number of taps is %d", m_coeffs[0].iwl(), m_coeffs[0].wl() -
    // m_coeffs[0].iwl(), m_accum.real().iwl(), m_accum.real().wl() -
    // m_accum.real().iwl(), coeffs.size());
}

template<class COEFF_T, class IO_T, class COEFFI_T>
bool FixedFIR<COEFF_T, IO_T, COEFFI_T>::input(const filter_io_t& data)
{
        assert(
        data.type == IO_TYPE_COMPLEX_FIXPOINT || data.type == IO_TYPE_FIXPOINT);
...
template<class COEFF_T, class IO_T, class COEFFI_T>
bool FixedFIR<COEFF_T, IO_T, COEFFI_T>::output(filter_io_t& data)
{
    if (m_outputReady) {
        m_outputReady = false;
        data          = m_output;
...
template<class COEFF_T, class IO_T, class COEFFI_T>
void FixedFIR<COEFF_T, IO_T, COEFFI_T>::tick()
{
...
template<class COEFF_T, class IO_T, class COEFFI_T>
void FixedFIR<COEFF_T, IO_T, COEFFI_T>::filter(IO_T & input,
                                               size_t polyPhaseOffset)
{
    ...
    if (!downSampling || (m_filterIteration >= decimationFactor)) {
        ...
        for (unsigned int coeffIdx = polyPhaseOffset; (coeffIdx < m_coeffs.size());
             coeffIdx += polyPhaseIncrement) {
...
template class FixedFIR<SLFixPoint, SLFixComplex, double>;
template class FixedFIR<SLFixPoint, SLFixPoint, double>;
template class FixedFIR<SLFixComplex, SLFixComplex, ComplexDouble>;
