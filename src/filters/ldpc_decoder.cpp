...
{
    if (m_softInputBits.size() >= m_hcols) { // we have enough bits to represent
                                             // a codeword
        ...
        if (didSolve) {
            ...
            //                std::cout << "LDPC decode success after " <<
            // solvedIterationNumber << " iterations" << std::endl;
            //            }
        } else {
            ...
            //            for (size_t i = 0; i < m_codeBits.size(); ++i) {
            //                std::cout <<
            // LLRToBit(m_codeBits[i].LLR.to_double());
        ...
        // the msg is sitting in the first part of the codeword, which is a
        // property of systematic codes
...
void LDPCDecoder::decode(size_t  iterations,
                         bool  & solved,
                         size_t& solved_iterations)
{
    ...
    for (size_t i = 0; i < iterations; ++i)
    {
        ...
        if (justSolved && !solved) {
            ...
            if (m_earlyExit) {
                return; // in hardware we may always complete worst-case
                        // iterations,
                        // but we'll exit early in software to speed up
                        // simulation
            }
        }
    }
...
{
    // Each row represents a parity check node, which in turn represents a
    ...
    for (unsigned i = 0; i < m_hrows; ++i)
    {
        for (unsigned j = 0; j < m_hcols; ++j)
        {
            if (m_H[i][j])
            {
                // Build adjacency lists for both check and bit nodes involved
                // in each connection
                ...
                // Store an LLR value for each edge connection to represent the
                // bit-to-check and check-to-bit messages
                GraphEdgeKey key(i, j);

                //                std::cout << "LDPC graph edge (" << i << ","
                // << j << ")" << std::endl;
                m_messages[key] = 0.0;
                m_tmpMsgs[key]  = 0.0;
            }
        ...
        //        std::cout << "Check node " << i << " has degree " <<
        // m_checkNodes[i].bits.size() << std::endl;
    }
}
...
{
    for (auto it = m_messages.begin(); it != m_messages.end(); ++it) {
        std::cout << "( check = " << it->first.checkNum << ", bit = " <<
        it->first.bitNum << ") = " << it->second.to_double() << std::endl;
    }
}

void LDPCDecoder::iteration()
{
    // Bits-to-checks pass: Create messages for each edge by taking the bit's
    // LLR given by the channel, summing
    // it with the estimates from the previous checks-to-bits pass, excluding
    // the information that was given from the target node
    for (size_t bit = 0; bit < m_codeBits.size(); ++bit) {
        ...
        for (size_t i = 0; i < m_codeBits[bit].checks.size(); ++i) {
            ...
            llr = llr.to_double() +
                  m_messages[GraphEdgeKey(currentCheckNode->checkNum,
                                          bit)].to_double();
        }

        // We need to disregard the information given by the particular node
        // we're updating. In the case of bits-to-checks, where we're summing up
        // the LLRs,
        // we can simply take the sum of all of the LLRs (from the loop above)
        // and subtract the value given by the particular check node.
        for (size_t i = 0; i < m_codeBits[bit].checks.size(); ++i) {
            CheckNode *currentCheckNode = m_codeBits[bit].checks[i];
            m_tmpMsgs[GraphEdgeKey(currentCheckNode->checkNum,
                                   bit)] = llr.to_double() -
                                           m_messages[GraphEdgeKey(
                                                          currentCheckNode->
                                                          checkNum,
                                                          bit)].to_double();
        }
    ...
    // Checks-to-bits pass: Create messages for each edge by taking the
    // minimum...
    for (size_t check = 0; check < m_checkNodes.size(); ++check) {
        ...
        for (size_t i = 0; i < m_checkNodes[check].bits.size(); ++i) {
            ...
            double msgVal =
                m_messages[GraphEdgeKey(check, currentBit->bitNum)].to_double();
        ...
        for (size_t i = 0; i < m_checkNodes[check].bits.size(); ++i) {
            ...
            // We need to disregard the information given by the particular node
            // we're updating. In the case of checks-to-bits, where we're
            // finding the minimum magnitude,
            // we can simply keep track of the second minimum magnitude and use
            // that only for the node which was responsible for the minimum
            // magnitude. The effect of the current
...
size_t LDPCDecoder::parityCheck() const
{
    ...
    for (size_t i = 0; i < m_checkNodes.size(); i++) {
        ...
        if (parity) { // odd number of ones
            //            std::cout << "equation #" << i << " does not meet
            // parity!" << std::endl;
            ++result;
        }
    }
...
std::vector<bool>LDPCDecoder::getHardCodeWord() const
{
    ...
    //    std::cout << "Assuming that the syndrome is 0, the message is: " <<
    // std::endl;
...
{
    for (size_t i = 0; i < m_codeBits.size(); i++) {
        std::cout << m_codeBits[i].LLR.to_int64() << ",";
    }
    std::cout << std::endl;
}
...
void LDPCDecoder::updateLLR()
{
    for (size_t i = 0; i < m_codeBits.size(); i++) {
        ...
        for (size_t j = 0; j < m_codeBits[i].checks.size(); ++j) {
            ...
            SLFixedPoint<LDPC_LLR_FORMAT> val =
                m_messages.at(GraphEdgeKey(check->checkNum, i));
            llr += (val).to_double();
        }
...
bool operator<(const LDPCDecoder::GraphEdgeKey& lhs,
               const LDPCDecoder::GraphEdgeKey& rhs)
{
