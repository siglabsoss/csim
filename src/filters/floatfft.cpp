...
void floatfftstage::init(int Ninput)
{
    ...
    memory = new std::complex<float> [N / 2];
...
void floatfftstage::butterfly(std::complex<float> array[2], std::complex<float> x,
        std::complex<float> y)
{
    array[0] = x + y;
    array[1] = x - y;
...
std::complex<float> floatfftstage::twiddler(int k)
{
//    if (!out3.is_open()){
...
//          }//If cannot read from file, return 1;
    ...
    fW_sin = -sin(ftheta);
    int theta;
 //   std::complex<float> W(fW_cos, fW_sin);
    //return W;
...
//    out3 << "Floating: " << fW_cos * 32768 << " " << fW_sin * 32768 << endl;
    ...
    std::complex<float> W(W_cos, W_sin);
...
void floatfftstage::output(std::complex<float> x)
{
    while (!next->ready) {
        //cout << N << "X" << endl;
        next->inputandtick(0);
...
void floatfftstage::inputandtick(std::complex<float> x)
{

    //	cout << "FFT(" << N << ") starting in state " << state << " with x " << x << endl;

    if (state != FFT_STATE_OUTPUT && x == std::complex<float>(0, 0)) {
        cout << "PROBLEMS " << endl;
    }

    std::complex<float> butterflyresult[2];
    std::complex<float> outputtemp;
...
//                outputtemp = memory[read_pointer]/std::complex<float> (32768,0) * twiddler(read_pointer);
//            }
//
//            else {
//                outputtemp = memory[read_pointer] * twiddler(read_pointer);
//                outputtemp =  outputtemp/std::complex<float> (32768,0);
//            }
//
...
void floatfftprint::inputandtick(std::complex<float> x)
{
...
void floatfftbuffer::inputandtick(std::complex<float> x)
{
    buf.push_back(x);
    count++;
}
...
void floatfft::inputandtick(std::complex<float> x)
{
