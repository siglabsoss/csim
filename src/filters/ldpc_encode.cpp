...
LDPCEncode::LDPCEncode(const std::vector<std::vector<bool> >& G) :
    FilterChainElement("LDPC_ENCODER", INPUT_BUFFER_BITS_MAX),
    m_G(G),
    m_outputBuffer()
{
            assert(m_G.size() > 0 && m_G[0].size() > 0);
    log_debug(
        "Constructed LDPC Encoder for message size of %d and codeword size of %d (rate = %f)",
        m_G.size(),
        m_G[0].size(),
        static_cast<double>(m_G.size()) / static_cast<double>(m_G[0].size()));
}
