...
void Biquad::init(const SOSCoeffs& coeffs)
{
    ...
    m_b[0].setFormat(m_coeffWidth,
                     maxIntLength,
                     SLFixPoint::QUANT_RND_HALF_UP,
                     SLFixPoint::OVERFLOW_SATURATE);
    m_b[0] = b0;

    m_b[1].setFormat(m_coeffWidth,
                     maxIntLength,
                     SLFixPoint::QUANT_RND_HALF_UP,
                     SLFixPoint::OVERFLOW_SATURATE);
    m_b[1] = b1;

    m_b[2].setFormat(m_coeffWidth,
                     maxIntLength,
                     SLFixPoint::QUANT_RND_HALF_UP,
                     SLFixPoint::OVERFLOW_SATURATE);
    m_b[2] = b2;

    m_a[0].setFormat(m_coeffWidth,
                     maxIntLength,
                     SLFixPoint::QUANT_RND_HALF_UP,
                     SLFixPoint::OVERFLOW_SATURATE);
    m_a[0] = a1;

    m_a[1].setFormat(m_coeffWidth,
                     maxIntLength,
                     SLFixPoint::QUANT_RND_HALF_UP,
                     SLFixPoint::OVERFLOW_SATURATE);
    m_a[1] = a2;
    ...
    log_debug("Coefficient format is Q(%d.%d)",
              maxIntLength,
              m_coeffWidth - maxIntLength);
    ...
    // Set the fixed point format of the acuumulator based on the coefficient
    // and input widths
    m_accum.setFormat(resultWidth, resultIntWidth, SLFixPoint::QUANT_RND_HALF_UP, SLFixPoint::OVERFLOW_SATURATE);

    log_debug("Accumulator format is (Q%d.%d)",
              m_accum.real().iwl(), m_accum.real().wl() - m_accum.real().iwl());
}
