                         ...
                         size_t       outputsPerPrint)
{
    ...
    while (outputCount < numBits) {
        if (chain.getFIFOUtilization() < 0.5) {
            ...
            sample.bit  = 1; // we can always use '1' because the scrambler will
                             // produce equal number of '1's and '0's
            chain.input(sample);
        }
...
static FilterChain constructLoopbackChain(double             noiseVar,
                                          size_t             syncDelay,
                                          const MCS          mcs,
                                          const std::string& down2CoeffFile,
                                          const std::string& down5CoeffFile,
                                          const std::string& up2CoeffFile,
                                          const std::string& up5CoeffFile,
                                          const std::string& HfFile,
                                          const std::string& ldpcH,
                                          const std::string& ldpcG)
{
    // Prepare coefficients
    std::vector<ComplexDouble> halfbandComplexCoeffs =
        utils::readComplexFromCSV<ComplexDouble>(down2CoeffFile);
    std::vector<ComplexDouble> by5ComplexCoeffs =
        utils::readComplexFromCSV<ComplexDouble>(down5CoeffFile);
    ...
    std::vector<ComplexDouble> up5ComplexCoeffs =
        utils::readComplexFromCSV<ComplexDouble>(up5CoeffFile);
                assert(halfbandComplexCoeffs.size() != 0 && by5ComplexCoeffs.size(
               ) != 0);
    ...
    std::vector<ComplexDouble> Hf = utils::readComplexFromCSV<ComplexDouble>(
        HfFile);
    ...
    CyclicPrefix *cp = new CyclicPrefix(FFT_SIZE,
                                        CP_SIZE,
                                        DUC_UPSAMPLE_FACTOR,
                                        mcs);
    DigitalUpConverter *duc = new DigitalUpConverter(-MIXER_FREQ,
                                                     up2Coeffs,
                                                     up5Coeffs);
    ...
    fft->setOutputFormat(FFT_OUTPUT_WL,
                         2,
                         SLFixPoint::QUANT_RND_HALF_UP,
                         SLFixPoint::OVERFLOW_SATURATE);
    ...
    FilterChain testChain =  *scramRx + *decode + *depunc + *demapper +
                            *fft_out + *ce + *fft + *fft_in + *fs + *ddc_out +
                            *ddc + *ne + *duc +
                            *duc_in + *cp + *ifft_out + *ifft + *ifft_in + *sm +
                            *mapper + *punc +
                            *encode + *scramTx;
#else // ifdef SHOULD_PROBE_FILTERS
    FilterChain testChain =  *scramRx + *decode + *depunc + *demapper + *ce +
                            *fft + *fs + *ddc + *ne + *duc + *cp + *ifft + *sm +
                            *mapper + *punc +
                            *encode + *scramTx;
...
static double getNoiseVarFromSNR(double snr)
{
    ...
    // http://read.pudn.com/downloads152/doc/comm/664022/ber.pdf

    // We are applying this noise in between the DUC/DDC. Because the DUC
    // actually has a
    // gain of 0.5, we account for that with a 3dB offset. In addition, because
    // this noise
    // is applied before the ~10dB processing gain that comes from the DDC's
    // bandpass, we
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Testing SNR of " << SNR_START << " to " << SNR_END <<
    " with coarse increments of " << SNR_COARSE_INCREMENT <<
    " and fine incrememnts of " << SNR_FINE_INCREMENT << std::endl;
    std::cout << "Counting up to " << MAX_BIT_ERRORS <<
    " bit errors per SNR or " << NUM_FRAMES_TO_TRANSMIT << " frames, " <<
    FRAME_SIZE << " bits long" << std::endl;
    ...
    while (snrVal <= SNR_END) {
        ...
        FilterChain loopback = constructLoopbackChain(noiseVar,
                                                      FRAME_SYNC_DELAY,
                                                      mcs,
                                                      down2,
                                                      down5,
        ...
        size_t numBitErrors;
        size_t numBitsOutput = runFilters(loopback,
                                          FRAME_SIZE * NUM_FRAMES_TO_TRANSMIT,
                                          numBitErrors,
                                          MAX_BIT_ERRORS,
                                          OUTPUTS_PER_PRINT);
        double ber = static_cast<double>(numBitErrors) /
                     static_cast<double>(numBitsOutput);
        bers[0].push_back(ber);
        ebnos[0].push_back(snrVal);

        if (numBitsOutput > 0) {
            std::cout << "SNR," << snrVal << ",ERR," << numBitErrors <<
            ",OUT," << numBitsOutput << ",BER," << ber << std::endl;
        } else {
