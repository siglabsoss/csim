...
int main(int argc, char *argv[])
{
    ...
    static constexpr double EBN_LOW = 0;
    static constexpr double EBN_HIGH = 15;
    ...
    Mapper::constellation_set_t scheme = Mapper::CONST_SET_QPSK;
...
//        std::cout << modSchemeToString(scheme) << ", " << ebn0 << ", " << ber << std::endl;
    ...
    for (double ebn0 = EBN_LOW; ebn0 <= EBN_HIGH; ebn0 += 1) {
        ...
        double theory = erfc(sqrt(0.5*pow(10, ebn0/10))) - pow(0.25*erfc(sqrt(0.5*pow(10, ebn0/10))),2);
    ...
    titles.push_back("QPSK Theory (MAYBE?)");
