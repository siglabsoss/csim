...
BOOST_AUTO_TEST_CASE(COMPLEX_FILTER)
{
    ...
    string data("./data/iirdata/data_in.csv");//Input data file
    ...
    string taps("./data/iirdata/taps.csv");
   ...
   typedef tokenizer< escaped_list_separator<char> > Tokenizer;
    ...
    string data3("./data/iirdata/data_out.csv");//Answers data file
   ...
   //		    BOOST_CHECK_MESSAGE(abs(output[k].imag/32768.00 -  imagAnswers[k] < .001) , output[k].imag/32768.00 << " is not the same as " << imagAnswers[k]);
   		    cout << output[k].real/32768.00 << " is not the same as " << realAnswers[k] << endl;
   	    }
