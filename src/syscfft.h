...
SC_MODULE(syscfft)
{
    ...
    sc_signal<sc_int<ADDR_SIZE> > addr_read, addr_write, register_r,
            register_im;
    ...
    sc_signal<sc_int<32> > result_0_r, result_0_im, result_1_r, result_1_im,
            comp_r_1, comp_im_1, scale_r, scale_im, ctrl_5;
...
#define FFT_STATE_IDLE (3)
    ...
    syscfft(sc_module_name nm, int Nin) :
            sc_module(nm)
    {
        N.write(Nin);
        N_STAGES = Nin;
        ...
        sensitive << en << read << clk << addr_read << addr_write << result_0_r
                << result_0_im << result_1_r << result_1_im << W_r << W_im
                << comp_im << comp_r << valid_Data_IN << valid_Data_OUT
                << x_in_r << x_in_im << write;
        ...
        if (Nin == 4) {
            sc_trace_file *wf = sc_create_vcd_trace_file("counter_1");
            ...
            sc_trace(wf, valid_Data_IN, "valid_Data_IN_1");
            sc_trace(wf, valid_Data_OUT, "valid_Data_OUT_1");
