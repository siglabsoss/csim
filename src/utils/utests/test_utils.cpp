...
CSIM_TEST_CASE(HAMMING_DISTANCE)
{
    ...
    unsigned int dist = utils::calculateHammingDistance(a, b);
    BOOST_CHECK_EQUAL(dist, 8);

    a = 0b10101010;
    b = 0b10101010;
    dist = utils::calculateHammingDistance(a, b);
...
CSIM_TEST_CASE(DYNAMIC_RANGE_SHIFT_CALCULATION)
{
    ...
    BOOST_CHECK_EQUAL(shiftBits, 2);

    shiftBits = utils::getShiftAmount(0.0);
    BOOST_CHECK_EQUAL(shiftBits, 0);
}
